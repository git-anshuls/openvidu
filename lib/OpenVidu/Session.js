...
var Session = /** @class */ (function (_super) {
    ...
    Session.prototype.connect = function (token, metadata) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (_this.openvidu.checkSystemRequirements()) {
                ...
                reject(new OpenViduError_1.OpenViduError(OpenViduError_1.OpenViduErrorName.BROWSER_NOT_SUPPORTED, "Browser " +
                    platform.name +
                    " (version " +
                    platform.version +
                    ") for " +
    ...
    Session.prototype.subscribe = function (stream, targetElement, param3, param4) {
        ...
        if (!!param3 && typeof param3 !== "function") {
            properties = {
                ...
                subscribeToAudio: typeof param3.subscribeToAudio !== "undefined"
                    ? param3.subscribeToAudio
                    : true,
                subscribeToVideo: typeof param3.subscribeToVideo !== "undefined"
    ...
    Session.prototype.forceDisconnect = function (connection) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.openvidu.sendRequest("forceDisconnect", { connectionId: connection.connectionId }, function (error, response) {
                if (error) {
                    logger.error("Error forcing disconnect for Connection " +
                    ...
                    logger.info("Forcing disconnect correctly for Connection " +
    ...
    Session.prototype.onParticipantLeft = function (msg) {
        var _this = this;
        this.getRemoteConnection(msg.connectionId, "Remote connection " +
            msg.connectionId +
            ...
            _this.ee.emitEvent("connectionDestroyed", [
                new ConnectionEvent_1.ConnectionEvent(false, _this, "connectionDestroyed", connection, msg.reason),
            ]);
        })["catch"](function (openViduError) {
    ...
    Session.prototype.onParticipantPublished = function (response) {
        ...
        var afterConnectionFound = function (connection) {
            ...
            if (!_this.remoteStreamsCreated[connection.stream.streamId]) {
                ...
                _this.ee.emitEvent("streamCreated", [
                    new StreamEvent_1.StreamEvent(false, _this, "streamCreated", connection.stream, ""),
                ]);
            }
        ...
        var connection;
        this.getRemoteConnection(response.id, "Remote connection '" +
            response.id +
    ...
    Session.prototype.onNewMessage = function (msg) {
        ...
        var strippedType = !!msg.type
            ? msg.type.replace(/^(signal:)/, "")
            : undefined;
        if (!!msg.from) {
            // Signal sent by other client
            this.getConnection(msg.from, "Connection '" +
                msg.from +
                ...
                _this.ee.emitEvent("signal", [
                    new SignalEvent_1.SignalEvent(_this, strippedType, msg.data, connection),
                ]);
                if (msg.type !== "signal") {
        ...
        var callback = function (connection) {
            if (!!connection.stream && connection.stream.streamId === msg.streamId) {
                ...
                _this.ee.emitEvent("streamPropertyChanged", [
                    new StreamPropertyChangedEvent_1.StreamPropertyChangedEvent(_this, stream, msg.property, msg.newValue, oldValue, msg.reason),
                ]);
                if (!!stream.streamManager) {
                    stream.streamManager.emitEvent("streamPropertyChanged", [
                        new StreamPropertyChangedEvent_1.StreamPropertyChangedEvent(stream.streamManager, stream, msg.property, msg.newValue, oldValue, msg.reason),
                    ]);
                }
            }
            else {
    ...
    Session.prototype.onNetworkQualityChangedChanged = function (msg) {
        if (msg.connectionId === this.connection.connectionId) {
            this.ee.emitEvent("networkQualityChanged", [
                new NetworkQualityChangedEvent_1.NetworkQualityChangedEvent(this, msg.newValue, msg.oldValue, msg.reason),
            ]);
        }
    };
    /**
    ...
    Session.prototype.recvIceCandidate = function (msg) {
        ...
        this.getConnection(msg.senderConnectionId, "Connection not found for connectionId " +
            ...
            stream
                .getWebRtcPeer()
                .addIceCandidate(candidate)["catch"](function (error) {
                logger.error("Error adding candidate for " +
                    stream.streamId +
                    " stream of endpoint " +
                    msg.endpointName +
                    ": " +
    ...
    Session.prototype.onRecordingStarted = function (response) {
        this.ee.emitEvent("recordingStarted", [
            new RecordingEvent_1.RecordingEvent(this, "recordingStarted", response.id, response.name),
        ]);
    };
    /**
     * @hidden
    ...
    Session.prototype.onRecordingStopped = function (response) {
        this.ee.emitEvent("recordingStopped", [
            new RecordingEvent_1.RecordingEvent(this, "recordingStopped", response.id, response.name, response.reason),
        ]);
    };
    /**
     * @hidden
                    ...
                    _this.openvidu.sendRequest("joinRoom", joinParams, function (error, response) {
                        if (!!error) {
                            ...
                            events_1.streams.forEach(function (stream) {
                                _this.ee.emitEvent("streamCreated", [
                                    new StreamEvent_1.StreamEvent(false, _this, "streamCreated", stream, ""),
                                ]);
                            });
                            resolve();
                        }
