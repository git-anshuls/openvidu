...
var StreamManager = /** @class */ (function (_super) {
        ...
        _this.canPlayListener = function () {
            if (_this.stream.isLocal()) {
                if (!_this.stream.displayMyRemote()) {
                    logger.info("Your local 'Stream' with id [" +
                        _this.stream.streamId +
                        "] video is now playing");
                    _this.ee.emitEvent("videoPlaying", [
                        new VideoElementEvent_1.VideoElementEvent(_this.videos[0].video, _this, "videoPlaying"),
                    ]);
                }
                else {
                    logger.info("Your own remote 'Stream' with id [" +
                        _this.stream.streamId +
                        "] video is now playing");
                    _this.ee.emitEvent("remoteVideoPlaying", [
                        new VideoElementEvent_1.VideoElementEvent(_this.videos[0].video, _this, "remoteVideoPlaying"),
                    ]);
                }
            }
            else {
                logger.info("Remote 'Stream' with id [" +
                    _this.stream.streamId +
                    "] video is now playing");
                _this.ee.emitEvent("videoPlaying", [
                    new VideoElementEvent_1.VideoElementEvent(_this.videos[0].video, _this, "videoPlaying"),
            ...
            _this.ee.emitEvent("streamPlaying", [
                new StreamManagerEvent_1.StreamManagerEvent(_this, "streamPlaying", undefined),
            ]);
        };
        return _this;
    }
    ...
    StreamManager.prototype.on = function (type, handler) {
        _super.prototype.onAux.call(this, type, "Event '" +
            type +
            "' triggered by '" +
        ...
        if (type === "videoElementCreated") {
            if (!!this.stream && this.lazyLaunchVideoElementCreatedEvent) {
                this.ee.emitEvent("videoElementCreated", [
                    new VideoElementEvent_1.VideoElementEvent(this.videos[0].video, this, "videoElementCreated"),
                ]);
        ...
        if (type === "streamPlaying" || type === "videoPlaying") {
            if (this.videos[0] &&
                ...
                this.ee.emitEvent("streamPlaying", [
                    new StreamManagerEvent_1.StreamManagerEvent(this, "streamPlaying", undefined),
                ]);
                this.ee.emitEvent("videoPlaying", [
                    new VideoElementEvent_1.VideoElementEvent(this.videos[0].video, this, "videoPlaying"),
                ]);
            }
        }
    ...
    StreamManager.prototype.once = function (type, handler) {
        _super.prototype.onceAux.call(this, type, "Event '" +
            type +
            "' triggered once by '" +
            (this.remote ? "Subscriber" : "Publisher") +
            "'", handler);
    ...
    StreamManager.prototype.createVideoElement = function (targetElement, insertMode) {
        ...
        if (typeof targetElement === "string") {
            targEl = document.getElementById(targetElement);
            if (!targEl) {
                throw new Error("The provided 'targetElement' couldn't be resolved to any HTML element: " +
                    targetElement);
            }
        }
        else if (targetElement instanceof HTMLElement) {
        ...
        this.ee.emitEvent("videoElementCreated", [
            new VideoElementEvent_1.VideoElementEvent(v.video, this, "videoElementCreated"),
        ]);
