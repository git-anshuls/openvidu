...
var Stream = /** @class */ (function (_super) {
    ...
    function Stream(session, options) {
        ...
        if (options.hasOwnProperty("id")) {
            ...
            if (!!_this.inboundStreamOpts.filter &&
                Object.keys(_this.inboundStreamOpts.filter).length > 0) {
                if (!!_this.inboundStreamOpts.filter.lastExecMethod &&
                    Object.keys(_this.inboundStreamOpts.filter.lastExecMethod).length === 0) {
    ...
    Stream.prototype.disposeMediaStream = function () {
        ...
        logger.info((!!this.outboundStreamOpts ? "Local " : "Remote ") +
    ...
    Stream.prototype.streamIceConnectionStateBroken = function () {
        ...
        if (this.isLocal &&
            ...
            logger.warn('OpenVidu Browser advanced configuration option "forceMediaReconnectionAfterNetworkDrop" is enabled. Publisher stream ' +
                this.streamId +
                "will force a reconnection");
            return true;
        }
        var iceConnectionState = this.getRTCPeerConnection()
            .iceConnectionState;
            ...
            var successCallback = function (sdpOfferParam) {
                ...
                _this.session.openvidu.sendRequest(method, params, function (error, response) {
                    if (error) {
                        ...
                        var needsTimeoutOnProcessAnswer = _this.session
        ...
        if (!!this.mediaStream) {
            if (this.streamManager instanceof Subscriber_1.Subscriber) {
                ...
                if (!!this.mediaStream.getAudioTracks()[0]) {
                    var enabled = !!this.streamManager.properties
                        .subscribeToAudio;
