...
export class WebRtcStats {
  ...
  public initWebRtcStats(): void {
    const elastestInstrumentation = localStorage.getItem(
        ...
        (stats) => {
          if (
            ...
            if (!!finalLocalCandidate) {
              ...
              finalLocalCandidate.raw = !!cand[0]
                ? cand[0].candidate
            ...
            if (!!finalRemoteCandidate) {
              ...
              finalRemoteCandidate.raw = !!cand[0]
                ? cand[0].candidate
  ...
  private sendStatsToHttpEndpoint(instrumentation): void {
        ...
        stats.forEach((stat) => {
          ...
          if (
            ...
            const metrics = {
              bytesReceived:
                (stat.bytesReceived -
                  this.stats.inbound[stat.mediaType].bytesReceived) /
                this.statsInterval,
              jitter: jit,
              packetsReceived:
                (stat.packetsReceived -
                  this.stats.inbound[stat.mediaType].packetsReceived) /
                this.statsInterval,
              packetsLost:
                (stat.packetsLost -
                  this.stats.inbound[stat.mediaType].packetsLost) /
                this.statsInterval,
            };
            const units = {
              bytesReceived: "bytes",
            ...
            if (stat.mediaType === "video") {
              metrics["framesDecoded"] =
                (stat.framesDecoded - this.stats.inbound.video.framesDecoded) /
                this.statsInterval;
              metrics["nackCount"] =
                (stat.nackCount - this.stats.inbound.video.nackCount) /
                this.statsInterval;
            ...
            const metrics = {
              bytesSent:
                (stat.bytesSent -
                  this.stats.outbound[stat.mediaType].bytesSent) /
                this.statsInterval,
              packetsSent:
                (stat.packetsSent -
                  this.stats.outbound[stat.mediaType].packetsSent) /
                this.statsInterval,
            };
            const units = {
              bytesSent: "bytes",
            ...
            if (stat.mediaType === "video") {
              metrics["framesEncoded"] =
                (stat.framesEncoded - this.stats.outbound.video.framesEncoded) /
                this.statsInterval;
          ...
          if (stat.type === "ssrc") {
            ...
            if (
              ...
              if (stat.mediaType === "video") {
                metrics["framesDecoded"] =
                  (stat.framesDecoded -
                    this.stats.inbound.video.framesDecoded) /
                  this.statsInterval;
                metrics["nackCount"] =
                  (stat.googNacksSent - this.stats.inbound.video.nackCount) /
                  this.statsInterval;
                units["framesDecoded"] = "frames";
                units["nackCount"] = "packets";
              ...
              if (stat.mediaType === "video") {
                metrics["framesEncoded"] =
                  (stat.framesEncoded -
                    this.stats.outbound.video.framesEncoded) /
                  this.statsInterval;
                units["framesEncoded"] = "frames";
    ...
    this.getStatsAgnostic(this.stream.getRTCPeerConnection(), f, (error) => {
      logger.log(error);
    });
  }
