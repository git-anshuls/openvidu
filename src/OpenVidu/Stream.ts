...
export class Stream extends EventDispatcher {
  ...
  constructor(
    session: Session,
    options: InboundStreamOptions | OutboundStreamOptions | {}
  ) {
    super();
    ...
    return new Promise((resolve, reject) => {
      ...
      this.session.openvidu.sendRequest(
        ...
        (error, response) => {
          if (error) {
            logger.error(
              "Error applying filter for Stream " + this.streamId,
              error
            );
            if (error.code === 401) {
              reject(
            ...
            logger.info(
              "Filter successfully applied on Stream " + this.streamId
            );
    ...
    return new Promise((resolve, reject) => {
      ...
      this.session.openvidu.sendRequest(
        ...
        (error, response) => {
          if (error) {
            logger.error(
              "Error removing filter for Stream " + this.streamId,
              error
            );
            if (error.code === 401) {
              reject(
            ...
            logger.info(
              "Filter successfully removed from Stream " + this.streamId
            );
  ...
  streamIceConnectionStateBroken() {
    ...
    const iceConnectionState: RTCIceConnectionState = this.getRTCPeerConnection()
      .iceConnectionState;
    return (
  ...
  initWebRtcPeerSend(reconnect: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const successCallback = (sdpOfferParam) => {
        logger.debug(
          "Sending SDP offer to publish as " + this.streamId,
          sdpOfferParam
        );
  ...
  initWebRtcPeerReceive(reconnect: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const successCallback = (sdpOfferParam) => {
        logger.debug(
          "Sending SDP offer to subscribe to " + this.streamId,
          sdpOfferParam
        );
        ...
        this.session.openvidu.sendRequest(method, params, (error, response) => {
          if (error) {
            ...
            const needsTimeoutOnProcessAnswer = this.session
              .countDownForIonicIosSubscribersActive;
            this.webRtcPeer
    ...
    if (!!this.mediaStream) {
      if (this.streamManager instanceof Subscriber) {
        ...
        if (!!this.mediaStream.getAudioTracks()[0]) {
          const enabled = !!(<Subscriber>this.streamManager).properties
            .subscribeToAudio;
