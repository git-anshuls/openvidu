    ...
    if (type === "streamCreated") {
      if (!!this.stream && this.stream.isLocalStreamPublished) {
        this.emitEvent("streamCreated", [
          new StreamEvent(false, this, "streamCreated", this.stream, ""),
        ]);
      } else {
  ...
  replaceTrack(track: MediaStreamTrack): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      if (this.stream.isLocalStreamPublished) {
        ...
        const senders: RTCRtpSender[] = this.stream
    ...
    return new Promise((resolve, reject) => {
      ...
      const successCallback = (mediaStream: MediaStream) => {
        ...
        if (!!mediaStream.getAudioTracks()[0]) {
          const enabled =
            this.stream.audioActive !== undefined &&
            this.stream.audioActive !== null
              ? this.stream.audioActive
              : !!this.stream.outboundStreamOpts.publisherProperties
                  .publishAudio;
          mediaStream.getAudioTracks()[0].enabled = enabled;
        }
        if (!!mediaStream.getVideoTracks()[0]) {
          const enabled =
            this.stream.videoActive !== undefined &&
            this.stream.videoActive !== null
          ...
          if (!this.stream.isSendScreen()) {
            ...
            this.videoReference.addEventListener("loadedmetadata", () => {
              ...
              this.screenShareResizeInterval = setInterval(() => {
                const firefoxSettings = mediaStream
                  .getVideoTracks()[0]
                  .getSettings();
                const newWidth =
                ...
                const newHeight =
                  platform.name === "Chrome" || platform.name === "Opera"
                    ? this.videoReference.videoHeight
                ...
                if (
                  ...
                  const oldValue = {
                    width: this.stream.videoDimensions.width,
                    height: this.stream.videoDimensions.height,
                  };
                  this.stream.videoDimensions = {
                    width: newWidth || 0,
                    height: newHeight || 0,
        ...
        if (this.stream.isSendScreen() && this.stream.isSendAudio()) {
          ...
          navigator.mediaDevices
            ...
            .catch((error) => {
              ...
              errorCallback(
                this.openvidu.generateAudioDeviceError(error, constraints)
              );
              return;
            });
        } else {
              ...
              .catch((e) => {
                if (error.constraint.toLowerCase() === "deviceid") {
                  ...
                  errorMessage =
                    ...
                    (<ConstrainDOMStringParameters>(
                      (<MediaTrackConstraints>constraints.video).deviceId!!
                    )).exact +
                    "' not found";
                } else {
                  errorName = OpenViduErrorName.PUBLISHER_PROPERTIES_ERROR;
      ...
      this.openvidu
        .generateMediaConstraints(this.properties)
        .then((myConstraints) => {
          if (
  ...
  initializeVideoReference(mediaStream: MediaStream) {
    ...
    if (!!this.firstVideoElement) {
      this.createVideoElement(
        ...
        <VideoInsertMode>this.properties.insertMode
      );
    }
