...
export class StreamManager extends EventDispatcher {
  ...
  constructor(stream: Stream, targetElement?: HTMLElement | string) {
    ...
    this.canPlayListener = () => {
      if (this.stream.isLocal()) {
        if (!this.stream.displayMyRemote()) {
          ...
          this.ee.emitEvent("videoPlaying", [
            new VideoElementEvent(this.videos[0].video, this, "videoPlaying"),
          ]);
        } else {
          logger.info(
      ...
      this.ee.emitEvent("streamPlaying", [
        new StreamManagerEvent(this, "streamPlaying", undefined),
      ]);
    };
  }
  ...
  createVideoElement(
    ...
    this.ee.emitEvent("videoElementCreated", [
      new VideoElementEvent(v.video, this, "videoElementCreated"),
    ]);
