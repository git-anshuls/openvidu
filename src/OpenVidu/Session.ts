...
import {
  ...
  NetworkQualityChangedReason,
} from "../OpenViduInternal/Events/NetworkQualityChangedEvent";
import {
  OpenViduError,
  OpenViduErrorName,
...
export class Session extends EventDispatcher {
    ...
    const afterConnectionFound = (connection) => {
      ...
      if (!this.remoteStreamsCreated[connection.stream.streamId]) {
        ...
        this.ee.emitEvent("streamCreated", [
          new StreamEvent(false, this, "streamCreated", connection.stream, ""),
        ]);
      }
  ...
  onNewMessage(msg): void {
    ...
    const strippedType: string = !!msg.type
    ...
    if (!!msg.from) {
      ...
      this.getConnection(
        ...
        .then((connection) => {
          this.ee.emitEvent("signal", [
            new SignalEvent(this, strippedType, msg.data, connection),
          ]);
          if (msg.type !== "signal") {
      ...
      this.ee.emitEvent("signal", [
        new SignalEvent(this, strippedType, msg.data, undefined),
      ]);
      if (msg.type !== "signal") {
        this.ee.emitEvent(msg.type, [
          new SignalEvent(this, strippedType, msg.data, undefined),
  ...
  onNetworkQualityChangedChanged(msg): void {
    if (msg.connectionId === this.connection.connectionId) {
      this.ee.emitEvent("networkQualityChanged", [
        new NetworkQualityChangedEvent(
          this,
          msg.newValue,
          msg.oldValue,
          msg.reason
  ...
  onRecordingStarted(response): void {
    this.ee.emitEvent("recordingStarted", [
      new RecordingEvent(this, "recordingStarted", response.id, response.name),
    ]);
  }
  ...
  onFilterEventDispatched(response): void {
    ...
    this.getConnection(
      ...
      stream.filter.handlers[response.eventType](
        new FilterEvent(stream.filter, response.eventType, response.data)
      );
    });
  }
  ...
  leave(forced: boolean, reason: string): void {
    ...
    if (!!this.connection) {
      ...
      if (!this.connection.disposed) {
        ...
        const sessionDisconnectEvent = new SessionDisconnectedEvent(
          this,
          reason
        );
          ...
          this.openvidu.sendRequest(
            ...
            (error, response) => {
              if (!!error) {
                ...
                const existingParticipants: ConnectionOptions[] =
                  response.value;
                ...
                events.streams.forEach((stream) => {
                  this.ee.emitEvent("streamCreated", [
                    new StreamEvent(false, this, "streamCreated", stream, ""),
                  ]);
                });
    ...
    return new Promise<Connection>((resolve, reject) => {
      ...
      if (!!connection) {
        ...
        if (this.connection.connectionId === connectionId) {
          ...
          reject(
            new OpenViduError(OpenViduErrorName.GENERIC_ERROR, errorMessage)
          );
        }
      }
    });
